/** @file hal/micro/cortexm3/compiler/arm.h
 * @brief arm for detailed documentation.
 *
 ...
 *  arm ARM C compiler.
 *
 * @note arm.h should be included first in all source files by setting the
 *  preprocessor macro PLATFORM_HEADER to point to it.  arm.h automatically
 *  includes platform-common.h.
 *
 *  See arm.h and platform-common.h for source code.
 *@{
 */
...
#else
  #define assert(expr) if(expr) (void)0  // to avoid warning with keil toolchain
...
#define NO_STRIPPING(var) var __attribute__((used))
#define NO_INIT(var) var __attribute__((zeroinit))
/**
...
#define EEPROM  errorerror
  ...
  /**
   * @brief __delay_cycles() is an intrinsic arm call; however, we
...
//#pragma arm section rodata = "FLASH_IMAGE"

//extern uint32_t FLASH_IMAGE;
extern uint32_t Image$$FLASH_IMAGE$$Base;
extern uint32_t Image$$FLASH_IMAGE$$Limit;

#define APPLICATIONIMAGE_START  &Image$$FLASH_IMAGE$$Base;
#define APPLICATIONIMAGE_END &Image$$FLASH_IMAGE$$Limit;

/**
 * @brief Set the application start and end address of RAM used
 */
#define RAM_START ((u32) __section_begin("RAM_BLOCK"))
#define RAM_END ((u32) __section_end ("RAM_BLOCK"))
...
/**
 * @brief Define __attribute__ to nothing since it isn't handled by arm.
 */
...
/**
 * @brief Define the magic value that is interpreted by arm C-SPY's Stack View.
 */
...
#define VAR_AT_SEGMENT(__variableDeclaration, __segmentName) \
   __variableDeclaration __attribute__((at(__segmentName)))  //__segmentName
...
//
// The segment names used here are the default segment names used by arm. Refer
// to the arm Compiler Reference Guide for a proper description of these
// segments.
//=============================================================================
#if 0
