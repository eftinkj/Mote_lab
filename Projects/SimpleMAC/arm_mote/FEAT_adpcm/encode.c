...
fprintf(stderr, "\tencode [-2|-3|4|5] [-a|u|l] < infile > outfile\n");
fprintf(stderr, "where:\n");
fprintf(stderr, "\t-2\tGenerate G.726 16kbps (2-bit) data\n");
...
fprintf(stderr, "\t-l\tProcess 16-bit linear PCM input data\n");
	...
	while (fread(in_buf, in_size, 1, stdin) == 1) {
		code_p = (*enc_routine)(in_size == 2 ? sample_short : sample_char,
		    in_coding, &state);
		resid = pack_output(code_p, enc_bits);
	}
