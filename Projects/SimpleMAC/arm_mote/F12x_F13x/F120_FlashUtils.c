...
#ifdef _FLASH_ENABLE_BYTE_WISE_CLEAR_
	void FLASH_Clear (FLADDR addr, unsigned numbytes, bit SFLE);
#endif //_FLASH_ENABLE_BYTE_WISE_CLEAR_
...
void FLASH_Clear (FLADDR dest, unsigned numbytes, bit SFLE)
{
   ...
   if (dest_1_page_end == dest_2_page_end) {
      ...
      wptr = FLASH_TEMP;
      rptr = dest_1_page_start;
      length = (unsigned int)(dest - dest_1_page_start);
      FLASH_Copy (wptr, 0, rptr, SFLE, length);
...
#endif // #ifdef _FLASH_ENABLE_BYTE_WISE_CLEAR_
...
// <dest>.
//
//void FLASH_Write (FLADDR dest, char *src, unsigned numbytes, bit SFLE) reentrant
void FLASH_Write (FLADDR dest, char xdata *src, unsigned numbytes, unsigned char SFLE) reentrant
{
   unsigned i;
   FLADDR tdst = dest;

   for (i = 0; i < numbytes; i++, tdst++) {
      FLASH_ByteWrite (tdst , src[i], (bit)SFLE);
   }
}
